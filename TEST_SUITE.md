...
As stated in the [Test Plan](EXERCISE-2.md) these tests focus on a subset of the Pet Store API - those related to the /pet set of endpoints and functionalities with a few omissions.
...
However for tests in ```test_pet_invoke_error_messages.py``` where all CRUD operations are expected to fail with error codes the creation of a pet dictionary in included in the test and not done via fixture. This is deliberate and is done to make it look obvious to a reader what invalid value is added and where.
...
The paths to endpoints covered in the test are separated from each test file and stored in a dictionary in utils/pet_api_urls.py. This is so that they can be (if necessary) amended without having to touch the test files themselves.
